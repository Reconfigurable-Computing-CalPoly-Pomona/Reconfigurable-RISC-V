# Runtime Tcl commands to interact with - core_wrapper_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/core_wrapper_v1_0_include.tcl
...
# Test: m_instr
set wdata_m_instr [get_init_data 0]
create_hw_axi_txn w_m_instr_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_instr}
create_hw_axi_txn r_m_instr_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_instr_addr
run_hw_axi w_m_instr_addr
run_hw_axi r_m_instr_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_instr_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - m_instr"
} else {
	puts "Data comparison test fail for - m_instr, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: m_data
set wdata_m_data [get_init_data 1]
create_hw_axi_txn w_m_data_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m_data}
create_hw_axi_txn r_m_data_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m_data_addr
run_hw_axi w_m_data_addr
run_hw_axi r_m_data_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m_data_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - m_data"
} else {
	puts "Data comparison test fail for - m_data, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
